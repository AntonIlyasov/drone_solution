...
class SimpleMover():
    ...
    def __init__(self):
        ...
        self.image_1 = np.zeros((320,240,1), dtype = "uint8")
        self.image_2 = np.zeros((320,240,1), dtype = "uint8")
    ...
    def camera_callback(self, msg):
        ...
        try:
            self.image_1 = self.cv_bridge.imgmsg_to_cv2(msg, "bgr8")
        except CvBridgeError as e:
            rospy.logerr("CvBridge Error: {0}".format(e))

        grey_image = cv2.cvtColor(self.image_1, cv2.COLOR_BGR2GRAY)
        _, mask = cv2.threshold(grey_image, 8, 255, cv2.THRESH_BINARY_INV)
        self.image_1 = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)

        cv2.line(self.image_1, (160, 0), (160, 240), (0, 123, 0), 1)
        cv2.line(self.image_1, (0, 120), (320, 120), (0, 123, 0), 1)
        ...
        if  (not np.isnan(np.average(top_points)) and not np.isnan(np.average(mid_points))):
            ...
            self.e_omega_z = self.omega_error
            # print(self.omega_error)
            
            cv2.circle(self.image_1, (top_line_point, 10), 5, (0,0,255), 1)
            cv2.circle(self.image_1, (mid_line_point, int(msg.height/2)), 5, (0,0,255), 1)
            cv2.line(self.image_1, (mid_line_point, int(msg.height/2)), (top_line_point, 10), (0, 0, 255), 3)
        ...
        if not np.isnan(np.average(cy_list)):
            ...
            # print(self.y_error)
            cv2.circle(self.image_1, (cy, int(msg.height/2)), 7, (0,255,0), 1)
            cv2.line(self.image_1, (160, 120), (cy, int(msg.height/2)), (0, 255, 0), 3)

        self.show_image(self.image_1)
    ...
    def camera_rings_callback(self, msg):
        ...
        try:
            self.image_2 = self.cv_bridge.imgmsg_to_cv2(msg, "bgr8")
        except CvBridgeError as e:
            rospy.logerr("CvBridge Error: {0}".format(e))
        ...
        self.image_2, red_pose, red_radius  = self.ring_detector(self.image_2, lower, upper, (0,0,255))


        # blue
        lower = np.uint8([40, 20, 20])
        upper = np.uint8([80, 50, 50])
        self.image_2, blue_pose, blue_radius = self.ring_detector(self.image_2, lower, upper, (255,0,0))
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                velocity = (self.drone_state[2] - self.alt_prev)/dt
                
                u_z = Kz*(self.des_alt - self.alt_prev) - Bz*velocity
                self.alt_prev = self.drone_state[2]

                # TODO: Steering control
                # Here!

                u_omega_z = Kw*self.e_omega_z - Bw*(self.e_omega_z - self.e_omega_prev)/dt
                # print(self.e_omega_z)
                self.e_omega_prev = self.e_omega_z

                # TODO: Offset control
                # Here!
                u_y = Ky*self.e_y - By*(self.e_y - self.e_y_prev)
                # print(self.e_y)
